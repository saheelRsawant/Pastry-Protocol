            ...
            match message with
                ...
                | Path(key, source, hops) ->
                    if peerId = key then
                        ...
                        while key.Length <> 0 && key.[i] = peerId.[i] do
                            i<- i+1
                        commonPrefixLength <- i
                        
                        let rtrow = commonPrefixLength
                        let mutable rtcol = Int32.Parse(key.[commonPrefixLength].ToString(), Globalization.NumberStyles.HexNumber)
                        if deadActors.Contains(routingTable.[rtrow, rtcol]) && ( rtcol <> 0) then
                            rtcol <- rtcol - 1

                        if isNull routingTable.[rtrow, rtcol] then
                            rtcol <- 0

                        if(peerId = "00" && rtcol = 0) then 
                            rtcol<- rtcol + 1

                        let foundKey = actorDict.TryFind routingTable.[rtrow, rtcol]
                        match foundKey with
                        | Some x->
                           x<! Path(key, source, hops+1)
                        | None ->
                            ()
